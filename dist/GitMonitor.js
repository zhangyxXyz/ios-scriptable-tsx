...
/********************************************************
 * author   :  seiun
 * date     :  6/11/2026, 05:16:27
 * build    :  2026-06-11 05:16:27
 * desc     :
 * version  :  1.0.0
...
var Widget = class extends WidgetBase {
        ...
        return /* @__PURE__ */ h(
          ...
          items.map((item) => {
            ...
            if (item.commit && item.commit.author) {
              ...
              authorName =
                item.author.name || item.author.login || item.author_name || "";
  ...
  async getData() {
    ...
    try {
      ...
      if (repoType === "github") {
        ...
        const token =
          this.repo.token ||
          this.currentSettings.basicSettings.defaultGithubToken.val;
        if (token) {
        ...
        const projectPath = encodeURIComponent(
          `${repoInfo.namespace}/${repoInfo.project}`,
        );
        ...
        const token =
          this.repo.token ||
          this.currentSettings.basicSettings.defaultGitlabToken.val;
        if (token) {
  ...
  generateRepoListHtml() {
    ...
    const defaultRepoName =
      this.settings.repoSettings?.defaultRepo ||
      this.settings.repo?.repoUrl ||
      "";
    let repoListHtml = "";
  ...
  async rebuildRepoList(webView, skipReload = false) {
    try {
      ...
      const escapedHtml = repoListHtml
        .replace(/\\/g, "\\\\")
        .replace(/`/g, "\\`")
        .replace(/\$/g, "\\$");
  ...
  getDefaultRepoDisplay() {
    ...
    if (this.settings.repoSettings?.defaultRepo)
      return this.settings.repoSettings.defaultRepo;
    if (this.currentSettings?.repoSettings?.defaultRepo?.val)
      return this.currentSettings.repoSettings.defaultRepo.val;
    return "请选择或者添加仓库";
  }
  async editRepo(index, repo) {
    try {
      ...
      if (editedRepo.repoUrl) {
        ...
        const isDefaultRepo =
          this.settings.repo && this.settings.repo.repoUrl === repo.repoUrl;
        if (isDefaultRepo) {
  ...
  Run() {
    try {
      ...
      if (
        this.currentSettings.repoSettings &&
        this.currentSettings.repoSettings.defaultRepo
      ) {
        this.currentSettings.repoSettings.defaultRepo.val =
          defaultRepoDisplayValue;
      }
    } catch (error) {
      console.log(error);
    }
    if (config.runsInApp) {
      ...
      this.registerSettingCategory("displaySettings", "显示设置", [
        ...
        },
        {
          ...
          option: {
            urlJumpType:
              this.currentSettings.displaySettings.urlJumpType.val ||
              "跳转至浏览器",
          },
          config: {
            selectOptions: [
      ...
      this.defaultRepoElementId = this.getSettingElementId(
        "actionSettings",
        "defaultRepo",
      );
    }
  ...
  decideGoto(commit) {
    switch (this.currentSettings.displaySettings.urlJumpType.val) {
      case "跳转至浏览器": {
        ...
        if (
          repoInfo &&
          (repoInfo.type === "gitlab" || this.repo.repoType === "gitlab")
        ) {
          let baseUrl;
          if (repoInfo.domain === "gitlab.com") {
            baseUrl = "https://gitlab.com";
