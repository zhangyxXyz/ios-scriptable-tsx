...
var CodexMonitor = class extends WidgetBase {
  ...
  saveAccountStore(store) {
    const accounts = store.accounts.filter(
      (account) => account?.accessToken && account?.accountName,
    );
  ...
  async requestUsage(credential) {
    ...
    try {
      ...
      if (statusCode2 > 0) throw new Error(`请求失败: HTTP ${statusCode2}`);
      throw error;
    }
    const statusCode = Number(request.response?.statusCode ?? 0);
  ...
  async presentAuthPage() {
    ...
    while (!closed) {
      ...
      if (action === "clear") {
        ...
        try {
          const credential = this.saveCredentialFromInput(
            form.authInput || "",
            form.accountName || "",
...
${account.accountId || "未识别 account id"}`;
  ...
  getSavedInfoHtml(current) {
    return current
      ? `Name: ${this.escapeHtml(current.accountName)}<br>Account: ${this.escapeHtml(current.accountId ?? "未识别")}<br>Expires: ${current.expiresAt ? this.escapeHtml(Utils.time("yyyy-MM-dd HH:mm", new Date(current.expiresAt * 1e3))) : "未知"}`
      : "未配置";
  }
  getAccountListHtml() {
    const store = this.getAccountStore();
    ...
    return store.accounts
      .map((account, index) => {
        ...
        const expires = account.expiresAt
          ? Utils.time("yyyy-MM-dd HH:mm", new Date(account.expiresAt * 1e3))
          : "未知";
...
window.renderAccounts=function(html){document.getElementById('accountList').innerHTML=html;}
  ...
  renderRowCard(parent, row, width) {
