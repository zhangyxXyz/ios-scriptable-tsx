...
var widgetStyleOptions = ["1", "2", "3", "4", "5", "6"];
var ChinaUnicom = class extends WidgetBase {
  constructor(scriptName) {
    ...
    this.logo =
      "https://raw.githubusercontent.com/anker1209/icon/main/zglt-big.png";
    this.smallLogo =
      "https://raw.githubusercontent.com/zhangyxXyz/PicGallery/master/IconSet/Scriptable/Application/ChinaUnicom.png";
    this.isRequestSuccess = false;
    this.errorMessage = "";
    this.data = [];
    this.flowColorHex = "#12A6E4";
    this.voiceColorHex = "#F86527";
    this.ringStackSize = 65;
    this.ringTextSize = 14;
    this.feeTextSize = 21;
    this.textSize = 13;
      ...
      },
      {
        ...
        onAction: async () => {
          ...
          if (!cookie) {
            await this.notify(
              "未读取到 Cookie",
              "请确认代理脚本或 BoxJS 已缓存 @ChinaUnicom.10010v4.cookie",
            );
            return;
          }
  ...
  parseWidgetStyle(value) {
    if (!value) return null;
    const style = value.trim();
    return widgetStyleOptions.includes(style) ? style : null;
  }
  cleanCookieStr(str) {
    return String(str)
      .split(";")
  ...
  async getProxyCookie() {
    const realtimeCookie = await this.requestJSON("http://10010v4.com/data")
      .then((res) => res.cookie)
      .catch((error) => {
        console.log(error);
        return "";
    ...
    const boxjsCookie = await this.requestJSON(
  ...
  getPercent(item, value) {
    ...
    if (!Number.isNaN(rawPercent) && rawPercent > 0)
      return Math.max(0, Math.min(100, 100 - rawPercent));
    const usedTitle = item.usedTitle || "";
    const usedMatch = usedTitle.match(/已用([\d.]+)%/);
    if (usedMatch)
      return Math.max(0, Math.min(100, 100 - Number(usedMatch[1])));
    ...
    if (item.type === "flow") {
      ...
      if (total > 0)
        return Math.max(
          0,
          Math.min(100, (this.toGB(value, item.unit || "") / total) * 100),
        );
    }
    if (item.type === "voice") {
  ...
  async getLogoImage() {
    return await this.$request.get(this.logo, "IMG");
  }
  async getSmallLogoImage() {
    return await this.$request.get(this.smallLogo, "IMG");
  }
  async header(stack) {
    const headerStack = stack.addStack();
    headerStack.addSpacer();
    const logo = headerStack.addImage(await this.getLogoImage());
    logo.imageSize = new Size(
      415 * this.logoScale * this.SCALE,
      125 * this.logoScale * this.SCALE,
    );
    headerStack.addSpacer();
    stack.addSpacer();
  ...
  async setForthWidget(widget) {
    ...
    mainStack.backgroundColor = Color.dynamic(
      new Color("#E2E2E7", 0.3),
      new Color("#2C2C2F", 1),
    );
  ...
  setList(stack, data) {
    ...
    unitStack.backgroundColor = Color.dynamic(
      data.iconColor,
      new Color(data.iconColor.hex, 0.3),
    );
  ...
  async small(stack, data, logo = false, en = false) {
    ...
    const iconImage = logo
      ...
      : rowStack.addImage(
          (SFSymbol.named(data.icon) || SFSymbol.named("phone.fill")).image,
        );
  ...
  async mediumCell(canvas, stack, data, color, fee = false, percent = false) {
    const bg = new LinearGradient();
    bg.locations = [0, 1];
    ...
    if (
      fee &&
      this.currentSettings.actionSettings.showUpdateTime.val === "显示"
    ) {
  ...
  async imageCell(canvas, stack, data, fee, percent = false) {
  ...
  ringContent(stack, data, percent = false) {
    ...
    rowUnit.addSpacer();
    [unit, number].forEach(
      (t) => (t.textColor = percent ? data.FGColor : this.widgetColor),
    );
  }
  ...
  gaugeChart(data, color) {
    ...
    this.drawHalfCircle(
      center.x + radius * Math.cos(startBgAngle),
      center.y + radius * Math.sin(startBgAngle),
      startBgAngle,
      circleRadius,
      drawing,
      fillColor,
    ...
    this.drawHalfCircle(
      center.x + radius * Math.cos(endBgAngle),
      center.y + radius * Math.sin(endBgAngle),
      endBgAngle,
      circleRadius,
      drawing,
      fillColor,
  ...
  drawHalfCircle(
    ...
    halfCirclePath.move(
      new Point(
        centerX + circleRadius * Math.cos(startAngle),
        centerY + circleRadius * Math.sin(startAngle),
      ),
    );
    for (let i = 0; i <= 10; i++) {
      const t = i / 10;
      const angle = startAngle + direction * Math.PI * t;
      halfCirclePath.addLine(
        new Point(
  ...
  nextRefreshDate() {
    const minutes =
      Number(this.currentSettings.actionSettings.refreshMinutes.val) || 10;
    return new Date(Date.now() + Math.max(1, minutes) * 60 * 1e3);
  }
  async render() {
