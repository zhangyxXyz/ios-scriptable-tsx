...
class Widget extends WidgetBase {
    ...
    renderStars(starCount, color) {
        ...
        for (let i = 0; i < fullStars; i++) {
            stars.push(h('wimage', { src: 'star.fill', width: starSize, height: starSize, filter: color, opacity: 0.9 }))
            if (i < 4) stars.push(h('wspacer', { length: 1 }))
        }
        if (hasHalfStar) {
            stars.push(h('wimage', { src: 'star.leadinghalf.filled', width: starSize, height: starSize, filter: color, opacity: 0.9 }))
            if (emptyStars > 0) stars.push(h('wspacer', { length: 1 }))
        }
        for (let i = 0; i < emptyStars; i++) {
            stars.push(h('wimage', { src: 'star', width: starSize, height: starSize, filter: color, opacity: 0.5 }))
