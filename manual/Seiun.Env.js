...
/**
 * @typedef {Object} SettingItemConfig - 类型特定配置对象
 *
 * // 样式相关
 * @property {string} [style] - 显示风格：'emphasis'|'compact'
 * @property {number} [truncateLength] - 截断长度（未配置使用默认值，<=0不截断）
 * @property {string} [defaultShowContent] - 默认显示内容（空值时显示）
 * @property {boolean} [useRawValue] - 是否使用原始值（从 baseSettings 读取，不走 settings[category]）
 *
 * // menu 类型特有
 * @property {Array<string>} [menuOptions] - 菜单选项
 *
 * // slider 类型特有
 * @property {number} [min] - 最小值
 * @property {number} [max] - 最大值
 * @property {number} [step] - 步长（默认1）
 * @property {string} [unit] - 单位（如 '%', '分钟'）
 *
 * // select 类型特有
 * @property {Array<{label:string, value:any}>} [selectOptions] - 选择器选项
 * @property {boolean} [multiple] - 是否多选（默认false）
 * @property {boolean} [editable] - 是否允许用户自定义增删选项（默认false）
 *
 * // text/password 类型特有
 * @property {string} [placeholder] - 输入框占位符
 *
 * // image 类型特有
 * @property {string} [imageSource] - 图片来源：'album'|'url'|'screenshot'|'both'
 * @property {boolean} [showThumbnail] - 是否显示缩略图（默认false）
 * @property {boolean} [clearable] - 是否可清空（默认true）
 * @property {boolean} [notifyOnSet] - 设置时是否通知（默认true）
 * @property {string} [setIcon] - 已设置状态图标名
 * @property {string} [emptyIcon] - 未设置状态图标名
 */
 ...
 *
 * === 显示层（必填）===
 * @property {string} [category='actionSettings'] - 分类，默认 'actionSettings'（主菜单）
 *   - 'actionSettings': 主菜单/组件配置（默认，等价于旧的 registerAction）
 *   - 其他值: 独立分类（等价于旧的 registerExtraSettingsCategoryItem）
 * @property {string} title - 标题
 * @property {string} [desc] - 描述
 * @property {Object|string} [icon] - 图标 {name, color} 或 URL 字符串
 *
 * === 数据层（可选，不填表示纯动作按钮）===
 * @property {'text'|'color'|'slider'|'select'|'password'|'image'|'menu'|'switch'} [type] - 数据类型 'reset'|'removeBackground' 为特殊类型，不参与设置保存
 * @property {Object} [option] - 数据键值对，如 {lightColor: '#000000'}（纯数据，不含配置）
 * @property {SettingItemConfig} [config] - 类型特定配置（样式、占位符、选项等）
 *
 * === 行为层（可选）===
 * @property {function(Object):void} [onAction] - 自定义点击行为
 *   - 不提供：使用默认 webview 输入界面（根据 type 自动处理）
 *   - 提供：完全接管点击行为，接收 option 作为参数（无 option 时为空对象）
 *
 * === 内部字段 ===
 * @property {string} [id] - 唯一标识符（自动生成，无需手动指定）
 */
...
class WidgetBase {
    ...
    setUserInfo = async () => {
        ...
        // 自定义操作处理器
        const customActionHandler = async (code, data, previewWebView) => {
            // 处理图片刷新事件（头像上传后触发）
     ...
     */
    loadSF2B64 = async (icon = 'square.grid.2x2', color = '#56A8D6', cornerWidthOrPure = 42, fontSize = 14) => {
        // 纯色模式：不带背景
    ...
    drawTableIcon = async (icon = 'square.grid.2x2', color = '#e8e8e8', cornerWidth = 42, iconSize = 160) => {
        const sfi = SFSymbol.named(icon)
    ...
    drawColorIcon = async colorValue => {
        ...
        for (let i = 0; i < segments; i++) {
            ...
            // HSL 颜色，色相从 0 到 360
            const hue = i
            const color = Color.dynamic(new Color(this.hslToHex(hue, 100, 50)), new Color(this.hslToHex(hue, 100, 50)))
    ...
    async presentSettings(
        ...
        for (const categoryKey of Object.keys(extraSettings)) {
            ...
            for (const item of category.items) {
                ...
                if (
                    ...
                    // 生成 HTML
                    const actionValue = item.type === 'customAction' && config.actionValue ? config.actionValue : null
                    ...
                    if (item.type === 'image') {
                        ...
                        const isDisabled = (optionKey === this.BACKGROUND_TRANSPARENT_KEY && (this.settings[this.BACKGROUND_DAY_KEY] || this.settings[this.BACKGROUND_NIGHT_KEY])) ||
                            ((optionKey === this.BACKGROUND_DAY_KEY || optionKey === this.BACKGROUND_NIGHT_KEY) && this.settings[this.BACKGROUND_TRANSPARENT_KEY])
                ...
                for (const key of keys) {
                    ...
                    let configval = config.useRawValue
                        ? item.option[key]
                        : this.settings.hasOwnProperty(targetCategoryForRead) && this.settings[targetCategoryForRead].hasOwnProperty(key)
                    ...
                    if (item.type === 'slider') {
                        ...
                        let customOptions = []
                        if (editable && this.settings[categoryKey] && this.settings[categoryKey][customOptionsKey]) {
                        ...
                        // select 指示器图标（上下箭头）
                        const selectIndicatorHtmlStr = '<img class="icon-select-indicator" src="' + selectIndicatorIcon + '"/>'
                        ...
                        const boolVal = configval === true || configval === 'true' || configval === 1 || configval === '1'
        ...
        const isBackgroundImage = (optionKey) => {
            return optionKey === this.BACKGROUND_DAY_KEY ||
        ...
        // 通用图片保存方法
        const saveImageCommon = async (img, optionKey, isGlobalSetting, notifyOnSet, actionItem, updateAvatar = false) => {
            if (!img) return false
            const storage = isGlobalSetting ? this.globalStorage : this.storage
        ...
        // 通用图片清除方法
        const clearImageCommon = async (optionKey, isGlobalSetting, notifyOnSet, actionItem, updateAvatar = false) => {
        ...
        // 通用图片URL保存方法
        const saveImageUrlCommon = async (imageUrl, optionKey, isGlobalSetting, notifyOnSet, actionItem, clearable = true) => {
            ...
            try {
                ...
                if (code.startsWith('image-menu-')) {
                    ...
                    if (actionItem && actionItem.type === 'image') {
                        ...
                        if (isBackgroundDisabled(optionKey)) {
                            const msg = optionKey === this.BACKGROUND_TRANSPARENT_KEY
                                ? '透明背景已被禁用\n\n请先清空日间/夜间背景图片'
                                : '日间/夜间背景已被禁用\n\n请先清空透明背景'
                        ...
                        if (selection === 'cancel') {
                            ...
                            try {
                                ...
                                if (backImage && (await this.verifyImage(backImage))) {
                                    await saveImageCommon(backImage, optionKey, isGlobalSetting, notifyOnSet, actionItem, true)
                                }
                            } catch (err) {
                            ...
                            const currentValue = isGlobalSetting ? this.baseSettings[optionKey] : this.settings[optionKey]
                ...
                if (code.startsWith('url-input-')) {
                    ...
                    if (actionItem && actionItem.type === 'image') {
                        ...
                        await saveImageUrlCommon(imageUrl, optionKey, isGlobalSetting, notifyOnSet, actionItem, clearable)
                    }
                    return
                }
                ...
                if (actionItem) {
                    ...
                    if (type === 'reset') {
                        ...
                        const saveImage = async (img) => {
                            return await saveImageCommon(img, optionKey, isGlobalSetting, notifyOnSet, actionItem, false)
                        }
                        ...
                        if (imageSource === 'screenshot') {
                            ...
                            // 在 WebView 中显示选择菜单
                            await showImageMenu(menuId, clearable
                                ? [
...
const Runing = async (Widget, default_args = '', isDebug = true, extra) => {
    ...
    if (config.runsInWidget) {
        ...
        if (!act || !M['_actions']) {
            ...
            if (M._settings.actionSettings && M._settings.actionSettings.items && M._settings.actionSettings.items.length > 0) {
...
class Storage {
    ...
    getCache(key, expirationMinutes) {
        ...
        if (expirationMinutes !== undefined && expirationMinutes !== null && expirationMinutes !== -1) {
            ...
            if (!createdAt || new Date() - createdAt > expirationMinutes * 60000) {
                removeCache(fullKey)
                return null
            }
        }
    ...
    clearAllStorage() {
        try {
            const baseStoragePath = this._fm.joinPath(this._fm.documentsDirectory(), 'storage')
            const namespacePath = this._nameSpace ? this._fm.joinPath(baseStoragePath, this._nameSpace) : baseStoragePath
        ...
        if (key.startsWith('base64:')) {
            ...
            if (this._urlMap[mapKey]) {
                ...
                if (!existingFileName.startsWith(expectedPrefix)) {
                    throw new Error(`Base64 签名冲突！签名 [${dataSignature}] 已被文件 [${existingFileName}] 占用。请为当前图片使用不同的 cacheKey（当前: ${cacheKey}）`)
                }
    ...
    async getFile(key, useCache = true, logable = true) {
        ...
        let isImage = this._isImageFile(fileName)
        let basePath = isImage ? this._imagesPath : this._dataPath
        let filePath = this._fm.joinPath(basePath, fileName)

        if (useCache) {
            if (this._fm.fileExists(filePath)) {
                try {
                    if (logable) console.log(`[Storage][getFile] Using cached ${isImage ? 'image' : 'file'}: ${fileName}`)
        ...
        try {
            if (logable) console.log(`[Storage][getFile] Downloading ${isImage ? 'image' : 'file'} online: ${fileName}`)
    ...
    async getImage(key, returnImage = false, useCache = true, logable = true, cacheKey = null) {
        const isUrl = isHttpUrl(key)
        ...
        if (isBase64) {
            ...
            try {
                ...
                if (logable) console.log(`[Storage][getImage] Converted and cached base64 image: ${finalCacheKey} [${dataSignature}]`)
                return returnImage ? img : this.imageToBase64(img, finalCacheKey)
            } catch (e) {
                console.error(`[Storage][getImage] Base64 image conversion failed: ${e}`)
                return null
        ...
        try {
            ...
            if (logable) console.log(`[Storage][getImage] Image loading completely failed, no cache available: ${fileName}`)
            return null
        }
    }
 ...
 */
async function loadPinyinDict(url) {
    const PINYIN_DICT_URL = url || 'https://raw.githubusercontent.com/zhangyxXyz/ios-scriptable/master/data/pinyinDict.js'
    const CACHE_KEY = 'pinyin_dict_cache'

    let cachedDict = globalStorage.getStorage(CACHE_KEY, -1)
    if (cachedDict) {
        // console.log('[loadPinyinDict] Using cached pinyin dictionary')
        globalThis.pinyinDict = cachedDict
        return
    }
...
function initPinyinUtils() {
    ...
    function buildDict() {
        if (!globalThis.pinyinDict) {
            if (!loading) {
                loading = true
                loadPinyinDict().catch(() => {}).finally(() => { loading = false })
            }
            return
        }
        ...
        dictNotone = {}
        for (let pinyin in globalThis.pinyinDict) {
            const characters = globalThis.pinyinDict[pinyin]
...
var pinyin_default = initPinyinUtils()
