...
// icon-color: cyan; icon-glyph: plane-departure;
...
class Widget extends DmYY {
    ...
    async loadBase64Img() {
        let req = new Request(this.icon_url)
        this.iconImage = await req.loadImage()
    }

    async getData() {
        const cached = await cache.getCache(this.airportName)
        if (cached) {
            this.data = cached
            return
        }
        try {
            var req = new Request(this.subUrl)
            req.method = 'GET'
            req.headers = {
            ...
            cache.setCache(this.airportName, data)
            this.data = data
    ...
    Run() {
        if (config.runsInApp) {
            ...
            this.registerAction(
                '账号设置',
                async () => {
                    await this.setAlertInput(`${this.name}账号`, '机场对应订阅信息\n多账号数据使用 "$" 连接\n通过传入参数所以选择配置', {
                        airportName: '机场名称',
                        subUrl: '订阅链接',
                        resetDay: '流量重置日期',
                        icon_url: '机场图标, base64编码, 选填'
                    })
                },
        ...
        try {
            ...
            var paramIdx = this.widgetParam ? parseInt(this.widgetParam) : 0
            this.airportName = airportName ? airportName.split('$')[paramIdx] || this.airportName : this.airportName
            this.subUrl = subUrl ? subUrl.split('$')[paramIdx] || this.subUrl : this.subUrl
            this.resetDay = resetDay ? parseInt(resetDay.split('$')[paramIdx]) || this.resetDay : this.resetDay
            this.icon_url = icon_url ? icon_url.split('$')[paramIdx] || this.icon_url : this.icon_url
    ...
    renderCommon = async w => {
        ...
        let item = bannerStack.addText(this.airportName)
        ...
        if (today.getDate() > this.resetDay) nextReset = new Date(nextReset.setMonth(nextReset.getMonth() + 1))
