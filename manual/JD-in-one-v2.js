...
class Widget extends DmYY {
    ...
    renderSmall = async w => {
        ...
        if (this.basicSetting.smallShowType === '个人信息') {
            ...
            } else if (this.funcSetting.showBaitiao === '打开' && this.baitiao.number > 0) {
    ...
    async setHeaderShow(stack, image) {
        ...
        if (this.widgetFamily === 'small') {
            ...
            avatarStack.size = new Size(this.basicSetting.logo * this.basicSetting.scale, this.basicSetting.logo * this.basicSetting.scale)
            avatarStack.cornerRadius = (avatarStack.size.width / 2) * this.basicSetting.scale
            avatarStack.backgroundImage = await this.getImageByUrl(
                this.basicSetting.customizeAvatar || this.userImage,
                `userImage_${this.userName}.png`
            )
            if (this.isPlus) {
                avatarStack.addImage(await this.getImageByUrl(this.plusFG, 'plusFGImage.png'))
            }
        } else {
    ...
    async setBeanShow(stack, textSize, imageSize) {
        ...
        this.rowBeanCell(yestodayStack, this.bean.ydayExpense.toString(), this.bean.ydayIncome.toString(), textSize, '昨日')
        beanStack.addSpacer()
        // 京豆图片
        ...
        this.rowBeanCell(todayStack, this.bean.todayExpense.toString(), this.bean.todayIncome.toString(), textSize, '今日')
    }
    ...
    getExipireBean = async () => {
        const dataName = '过期京豆'
        const url = 'https://wq.jd.com/activep3/singjd/queryexpirejingdou?_=g_login_type=1&sceneval=2'
        const options = {
            headers: {
                cookie: this.cookie,
        ...
        try {
            let data = await this.httpRequest(dataName, url, false, options, 'exipireBeanData')
            if (data) {
                ...
                if (data.ret === 0) {
                    for (let i = 0; i < data['expirejingdou'].length; i++) {
                        if (data['expirejingdou'][i]['expireamount'] > 0) {
                            ...
                            this.expireBean.desc = this.timeFormat(data['expirejingdou'][i]['time'] * 1000)
                            break
                        }
                    }
                } else {
    ...
    run = (filename, args) => {
        ...
        Object.keys(this.basicSetting).forEach(key => {
            if (!this.settings.basicSetting.hasOwnProperty(key)) this.settings['basicSetting'][key] = this.basicSetting[key]
        })
        if (!this.settings.chartSetting) this.settings.chartSetting = this.chartSetting
        Object.keys(this.chartSetting).forEach(key => {
            if (!this.settings.chartSetting.hasOwnProperty(key)) this.settings['chartSetting'][key] = this.chartSetting[key]
        })
        if (!this.settings.funcSetting) this.settings.funcSetting = this.funcSetting
        Object.keys(this.funcSetting).forEach(key => {
            if (!this.settings.funcSetting.hasOwnProperty(key)) this.settings['funcSetting'][key] = this.funcSetting[key]
        })
        ...
        Object.keys(this.settings['basicSetting']).forEach(key => {
            if (key == 'customizeName' || key == 'customizeAvatar' || key == 'smallShowType' || key == 'walletShowType') {
        ...
        this.JDindex = typeof args.widgetParameter === 'string' ? parseInt(args.widgetParameter) : false
        try {
