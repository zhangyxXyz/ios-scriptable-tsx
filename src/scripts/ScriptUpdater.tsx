...
EndAwait(async () => {
    ...
    getSubscriptions() {
        ...
        const urls = Array.isArray(fromSettings) && fromSettings.length ? fromSettings : fromCurrent
        if (!urls || urls.length === 0) return [DEFAULT_SUBSCRIPTION_URL]
        return [...new Set((urls || []).map(url => String(url).trim()).filter(Boolean))]
    }

    saveSubscriptions(urls: string[]) {
    ...
    readHeaderField(source: string, field: string) {
        const match = source.match(new RegExp(`^\\s*\\*\\s*${field}\\s*:\\s*(.*)$`, 'm'))
        return match ? match[1].trim() : ''
    }

    resolveRawUrl(manifestUrl: string, manifest: SubscriptionManifest, script: SubscriptionScript) {
        if (/^https?:\/\//.test(script.rawUrl)) return script.rawUrl
        const baseUrl = manifest.rawBaseUrl || manifestUrl.replace(/\/[^/]*$/, '')
        return `${baseUrl.replace(/\/+$/, '')}/${script.rawUrl || script.fileName}`
    }

    async fetchManifest(url: string) {
        const req = new Request(url)
        const manifest = (await req.loadJSON()) as SubscriptionManifest
    ...
    async buildState(message = ''): Promise<SubscriptionState> {
    ...
    shouldUpdateScript(script: SubscriptionScript, force = false): UpdateDecision {
    ...
    async downloadScript(script: SubscriptionScript, force = false) {
        ...
        if (!decision.shouldUpdate) return `${script.name?.zh || script.fileName} ${decision.reason}`
    ...
    findScript(state: SubscriptionState, fileName: string) {
        return state.manifests.flatMap(item => item.scripts).find(script => script.fileName === fileName)
    }
    ...
    renderManagerHtml(state: SubscriptionState) {
...
render();
    ...
    async waitForBridgeEvent(webView: WebView) {
        ...
        while (true) {
            ...
            try {
                if (event.code === 'addSubscription') {
                    this.saveSubscriptions([...state.subscriptions, String(event.data || '')])
                    state = await this.buildState('订阅已添加')
                } else if (event.code === 'removeSubscription') {
                    this.saveSubscriptions(state.subscriptions.filter(url => url !== event.data))
                    ...
                    const messages = []
                    for (const script of scripts) messages.push(await this.downloadScript(script))
                    const updatedCount = messages.filter(message => !message.includes('无需更新')).length
                    state = await this.buildState(updatedCount ? `已更新 ${updatedCount} 个脚本` : '全部脚本无需更新')
                } else if (event.code === 'forceUpdateAll') {
                    const scripts = state.manifests.flatMap(item => item.scripts)
                    for (const script of scripts) await this.downloadScript(script, true)
                    state = await this.buildState(`已强制更新 ${scripts.length} 个脚本`)
                }
            } catch (error) {
