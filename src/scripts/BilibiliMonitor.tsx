...
type BilibiliSettings = {
    basicSettings: {
        ridType: SettingValue<string>
        urlJumpType: SettingValue<string>
    }
    displaySettings: {
        mediaWidgetShowDataNum: SettingValue<number>
        largeWidgetShowDataNum: SettingValue<number>
        listDataColorShowType: SettingValue<string>
        listDataUpdateTimeShowType: SettingValue<string>
    }
}
...
const rankTypeOptions = ['全站', '动画', '音乐', '游戏', '娱乐', '科技', '鬼畜', '舞蹈'] as const
type RankType = (typeof rankTypeOptions)[number]

const rankTypes: Record<RankType, number> = {
    全站: 0,
    动画: 1,
    音乐: 3,
    游戏: 4,
...
class BilibiliMonitor extends WidgetBase {
    name = 'B站榜单'
    en = 'BilibiliMonitor'
    widgetParam = args.widgetParameter
    contentRowSpacing = 5
    httpData: RankingResponse | null = null
    isRequestSuccess = false
    ridTypeDic = rankTypes

    currentSettings: BilibiliSettings = {
        basicSettings: {
    ...
    async getData() {
        ...
        try {
            const settingRid = this.currentSettings.basicSettings.ridType.val as RankType
            const realRidId = this.widgetParam ? parseInt(this.widgetParam) : this.ridTypeDic[settingRid] || 0
        ...
        this.registerSettingCategory('basicSettings', '基础设置', [
            {
                ...
                option: {ridType: '全站'},
                config: {
                    selectOptions: rankTypeOptions.map(item => ({label: item, value: item})),
    ...
    async renderCommon(widget: ListWidget) {
        if (!this.httpData || !(this.httpData.code === 0 || this.httpData.code === '0')) return
        ...
        const items = this.httpData.data.splice(0, Math.min(itemCount, this.httpData.data.length))
        items.forEach(item => console.log(`• ${item.title}`))

        GenrateView.setListWidget(widget)
        return (
            <wbox spacing={this.contentRowSpacing}>
                <wspacer length={this.contentRowSpacing} />
                ...
                {items.map(item => {
                    const itemColor =
                        this.currentSettings.displaySettings.listDataColorShowType.val === '随机颜色'
                            ...
                            : this.widgetColor
                    return (
                        <wstack verticalAlign="center" href={this.decideGoto(item)}>
                            <wtext textColor={itemColor} font={Font.mediumSystemFont(12)} maxLine={1}>
                                {`• ${item.title}`}
                            </wtext>
                            <wspacer />
                            <wtext textColor={itemColor} font={Font.systemFont(11)} opacity={0.6} textAlign="right">
                                {item.name || ''}
                            </wtext>
                        </wstack>
                    )
                ...
                {this.currentSettings.displaySettings.listDataUpdateTimeShowType.val === '显示' && (
                    <wstack verticalAlign="center" padding={[0, 0, 5, 0]}>
                        <wspacer />
