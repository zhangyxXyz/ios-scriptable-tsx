...
const {WidgetBase, Runing, GenrateView, h, Utils, Storage} = runtimeRequire(dependencyFileName) as SeiunEnv & {
    Storage: new (name: string) => WidgetStorage
}
...
type InfoCollectionSettings = {
    accountSettings: {
        ...
        weatherKey: SettingValue<string>
        tencentApiKey: SettingValue<string>
        lockLocation: SettingValue<string>
    }
    displaySettings: {
        lunarInfoColorHex: SettingValue<string>
        honeyInfoColorHex: SettingValue<string>
        weatherInfoColorHex: SettingValue<string>
        batteryInfoColorHex: SettingValue<string>
        yearProgressColorHex: SettingValue<string>
...
class InfoCollection extends WidgetBase {
    ...
    areaInfo: AreaInfo | null = null
    lunarInfo: LunarInfo | null = null
    weatherInfo: WeatherInfo | null = null
    honeyInfo: HoneyInfo | null = null

    currentSettings: InfoCollectionSettings = {
        accountSettings: {
    ...
    async getLocation() {
        this.locationInfo = storage.getStorage<LocationInfo>('location')
        if (this.currentSettings.accountSettings.lockLocation.val === '锁定' && this.locationInfo) {
            console.log('[+]位置已锁定，使用缓存数据')
        } else {
            try {
                const currentLocation = (await Location.current()) as LocationInfo
    ...
    async getLocationArea() {
        const storageArea = storage.getStorage<AreaInfo>('area', 1)
        if (storageArea) {
            ...
            try {
                ...
                const area = await this.$request.get<AreaInfo>({
                    url: areaReqUrl,
    ...
    async getLunar(day: number) {
        try {
            ...
            const webview = new WebView() as WebView & WebViewEvaluate
    ...
    renderYearProgress() {
        ...
        return this.renderProgress((now.getTime() - start.getTime()) / (end.getTime() - start.getTime()))
    }
