...
class Widget extends WidgetBase {
    ...
    extractInnerHtmlById(html: string, id: string) {
        ...
        const closeMatch = source.slice(idIndex).match(new RegExp(`<\\/${tagName}>`, 'i'))
    ...
    parseMovieDataFromHtml(html: string): MovieData | null {
        ...
        const backgroundMatch = bgimgHtml.match(/background-image:\s*url\(["']?(.+?)["']?\)/i)
        const movieImg = this.extractAttr(movieImgHtml, 'src') || (backgroundMatch ? this.decodeHtmlText(backgroundMatch[1]) : '')
        const movieDesc = this.extractText(this.extractInnerHtmlById(html, 'movie-text') || movieTextHtml)
        const movieName = this.extractText(this.extractInnerHtmlById(html, 'movie-name') || movieNameHtml).replaceAll('——', '')
        let movieInformation = this.extractText(this.extractInnerHtmlById(html, 'movie-information') || movieInfoHtml)
        const movieRating = movieInformation.slice(0, 3)
        movieInformation = movieInformation.slice(5)
        const movieLink = this.extractAttr(movieLinkHtml, 'href') || this.extractAttr(movieNameHtml, 'href')
    ...
    generateStars(movieRating: string | number, starSize = 18) {
        ...
        for (let index = 0; index < fillCount; index++) {
            totalCount += 1
            stars.push(/* @__PURE__ */ h('wimage', { src: fillStar, width: starSize, height: starSize, filter: ratingColor }))
        ...
        if (remainCount >= 0.5) {
            ...
            stars.push(/* @__PURE__ */ h('wimage', { src: halfStar || emptyStar, width: starSize, height: starSize, filter: ratingColor }))
        }

        for (let index = 0; index < 5 - totalCount; index++) {
            stars.push(/* @__PURE__ */ h('wspacer', { length: 2 }))
            stars.push(/* @__PURE__ */ h('wimage', { src: emptyStar, width: starSize, height: starSize, filter: ratingColor, opacity: 0.5 }))
        }
    ...
    generateMovieInfo(
        ...
        return /* @__PURE__ */ h(
            ...
            /* @__PURE__ */ h('wtext', { textColor: data.titleColor, font: Font.mediumSystemFont(titleFontSize) }, data.movieName),
            ...
            /* @__PURE__ */ h(
                ...
                /* @__PURE__ */ h('wtext', { textColor: data.subTitleColor, font: Font.semiboldSystemFont(ratingTextSize) }, ratingText)
            ),
            /* @__PURE__ */ h('wspacer', { length: starSize >= 18 ? 6 : 4 }),
            /* @__PURE__ */ h('wtext', { textColor: data.subTitleColor, font: Font.semiboldSystemFont(subTitleFontSize) }, data.movieInformation)
        )
    }
    ...
    async generatePosterBackground(image: Image | null, widgetSize: WidgetSizeSpec, posterRatio: number) {
        ...
        try {
            ...
            cropCtx.drawImageInRect(image, new Rect(-cropX, -cropY, image.size.width, image.size.height))
            const croppedImage = cropCtx.getImage()

            ctx.drawImageInRect(croppedImage, new Rect(posterX, 0, posterWidth, posterHeight))
    ...
    renderCommon = async (w: ListWidget, sizeSplit: number, descSpan: number, useMoviePoster4FullBG: boolean, name: string) => {
        ...
        if (this.currentSettings.displaySettings.autoTimeColor.val && data.image) {
        ...
        return /* @__PURE__ */ h(
            'wbox',
            {
                background: useMoviePoster4FullBG ? data.image : (this.backGroundColor || new Color('#000000')),
                padding: [0, 0, 0, 0],
            },
            /* @__PURE__ */ h(
                'wstack',
    ...
    renderSmall = async (w: ListWidget) => {
        ...
        const backgroundImg = await this.generatePosterBackground(data.image, widgetSize, 0.35)

        GenrateView.setListWidget(w)
        return /* @__PURE__ */ h(
            'wbox',
            {
                background: backgroundImg || (this.backGroundColor || new Color('#000000')),
                padding: [8, 8, 8, 8],
            },
