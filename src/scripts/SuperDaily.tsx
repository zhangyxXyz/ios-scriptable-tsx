...
const solarTermCenturyValues = [
    3.87, 18.73, 5.63, 20.646, 4.81, 20.1, 5.52, 21.04, 5.678, 21.37, 6.5, 22.2, 7.928, 23.65, 8.35, 23.95,
...
class SuperDaily extends WidgetBase {
    ...
    getSolarTermDate(year: number, termIndex: number) {
        const yearInCentury = year % 100
        const day =
            Math.floor(yearInCentury * 0.2422 + solarTermCenturyValues[termIndex]) - Math.floor((yearInCentury - 1) / 4)
    ...
    getSolarTermTheme(date: Date): SolarTermTheme | null {
        ...
        const year = date.getFullYear()
        const index = solarTerms.findIndex((_, termIndex) => this.getSolarTermDate(year, termIndex) === dateKey)
