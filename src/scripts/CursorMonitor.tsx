...
import type {SeiunEnv, WidgetStorage} from '@app/env/types'
...
const parseTokenCount = (value?: string | number) => parseInt(String(value ?? 0))

type DisplayMode = '每日用量' | '总用量' | '套餐概览'
type SettingValue<T> = {val: T; type?: string}
type SettingGroup<T extends Record<string, unknown>> = {
    [K in keyof T]: SettingValue<T[K]>
}
type CursorCurrentSettings = {
    accountSettings: SettingGroup<{defaultAccount: string}>
    dailyDisplaySettings: SettingGroup<{
        displayMode?: string
        mediaWidgetShowDataNum: number
        largeWidgetShowDataNum: number
        listDataColorShowType: string
        listDataUpdateTimeShowType: string
    }>
    aggregatedDisplaySettings: SettingGroup<{
        mediaWidgetShowDataNum: number
        largeWidgetShowDataNum: number
        showAggregatedPercentage: boolean
        listDataColorShowType: string
        listDataUpdateTimeShowType: string
    }>
    basicSettings: SettingGroup<{displayMode: DisplayMode}>
}
type CursorAccount = {
    accountName?: string
    cookie?: string
...
type UsageSummary = {
    billingCycleStart?: string | number | Date
    billingCycleEnd?: string | number | Date
...
type AggregatedData = {
    aggregations: AggregatedUsageItem[]
...
type CursorSettingItem = {
    ...
    onAction?: (webView: WebViewLike) => boolean | void | Promise<boolean | void>
}
type CursorRuntime = {
    settings: CursorSettings
    currentSettings: CursorCurrentSettings
    defaultAccountElementId?: string
    ...
    insertTextByElementId(webView: WebViewLike, elementId: string, text: string): Promise<void>
    getSettingElementId(category: string, key: string): string
    setAlertInput(
        title: string,
        message: string,
    ...
    storage: WidgetStorage & {
        ...
        getImage(url: string, useCache?: boolean, failReturnDefault?: boolean, setColor?: boolean): Promise<Image | null>
    }
    registerSetting(items: CursorSettingItem | CursorSettingItem[]): void
    registerSettingCategory(category: string, title: string, items: CursorSettingItem[]): void
}

class CursorMonitor extends WidgetBase {
    ...
    aggregatedData: AggregatedData | null = null
    usageSummary: UsageSummary | null = null
    isRequestSuccess = false
    dataFetchTime: string | Date | null = null
    isDataExpired = false
    paramDisplayMode: string | null = null
    paramAccountIndex: number | null = null
    currentAccount: CursorAccount | null = null

    displayModeOptions: Array<{label: DisplayMode; value: DisplayMode}> = [
    ...
    async resetPendingAction(webView: WebViewLike) {
        try {
    ...
    async readPendingAccountAction(webView: WebViewLike) {
        return await webView.evaluateJavaScript<string>(
            `(function() {
    ...
    async updateSettingDisplay(webView: WebViewLike | null, category: string, key: string, text: string) {
    ...
    async bindAccountActions(webView: WebViewLike) {
        try {
    ...
    async rebuildAccountList(webView: WebViewLike, skipReload = false) {
        try {
            if (!skipReload) {
            ...
            await new Promise(resolve => Timer.schedule(150, false, () => resolve(undefined)))
                ...
                if (result.action === 'accountClick') {
                    ...
                    try {
                        ...
                        if (account && account.accountName && account.cookie) {
                            ...
                            this.settings.dataSource = this.settings.dataSource.filter(Boolean)
                            this.saveSettings()
                            await this.rebuildAccountList(webView)
                            await this.resetPendingAction(webView)
                        }
    ...
    async editAccount(index: number, account: CursorAccount) {
        try {
            const alert = new Alert()
            alert.title = '修改账号'
            ...
            alert.addTextField('团队 ID（个人账号使用 -1）', String(account.teamId || -1))
            alert.addAction('确定')
            alert.addCancelAction('取消')
            const result = await alert.presentAlert()
            if (result === -1) return false

            const editedAccount: CursorAccount = {
                accountName: alert.textFieldValue(0).trim(),
                cookie: alert.textFieldValue(1).trim(),
                teamId: alert.textFieldValue(2).trim() || '-1',
            }
            ...
            if (editedAccount.accountName && editedAccount.cookie) {
                ...
                const isDefaultAccount = this.settings.account && this.settings.account.accountName === account.accountName
                if (isDefaultAccount) {
    ...
    async copyAccount(index: number, account: CursorAccount) {
        try {
            const copiedAccount = {
    ...
    async deleteAccount(index: number, account: CursorAccount) {
        try {
            const alert = new Alert()
            alert.title = '确认删除'
    ...
    formatMembershipType(membershipType: string) {
    ...
    async requestWithCache<T>(
        ...
        const isPlaygroundMockResponse = (value: unknown) => {
        ...
        try {
            // 尝试从缓存获取
            const cachedData = this.storage.getStorage<T>(finalCacheKey, this.storageExpirationMinutes)
            if (cachedData) {
            ...
            // 尝试使用缓存数据（不限制时间）
            const cachedData = this.storage.getStorage<T>(finalCacheKey)
            if (cachedData) {
                ...
                if (cacheTime) {
                    this.dataFetchTime = cacheTime
                    const now = new Date()
                    this.isDataExpired = (now.getTime() - new Date(cacheTime).getTime()) > (this.storageExpirationMinutes * 60000)
                } else {
                    this.dataFetchTime = new Date()
    ...
    async getUsageSummary() {
        try {
            ...
            const data = await this.requestWithCache<UsageSummary>(
                "usageSummary",
    ...
    async getDailyUsageData() {
        ...
        try {
            ...
            const data = await this.requestWithCache<UsageData>(
                'dailyUsage',
            ...
            // 尝试使用缓存数据
            const cachedData = this.storage.getStorage<UsageData>('dailyUsage')
            if (cachedData) {
    ...
    async getAggregatedData() {
        ...
        try {
            ...
            const data = await this.requestWithCache<AggregatedData>(
                'aggregatedUsage',
            ...
            if (data && data.aggregations) {
                ...
                this.isRequestSuccess = true
                // 总量的时候最后额外输出
                const totalTokens = (parseTokenCount(data.totalInputTokens) +
            ...
            // 尝试使用缓存数据
            const cachedData = this.storage.getStorage<AggregatedData>('aggregatedUsage')
            if (cachedData) {
    ...
    formatSpend(cents: number) {
        return (cents / 100).toFixed(2)
    }

    formatOnDemandUsage(onDemandUsage?: OnDemandUsage) {
    ...
    formatTokens(tokens: string | number) {
    ...
    groupByDay(dailySpend: DailySpendItem[]) {
        const grouped: Record<string, DailySpendItem[]> = {}
    ...
    calculateDayTotal(items: DailySpendItem[]) {
    ...
    Run() {
        ...
        if (config.runsInApp) {
            ...
            this.registerSetting({
                ...
                onAction: async (parentWebView: WebViewLike) => {
    ...
    getMaxSpend(days: string[], grouped: Record<string, DailySpendItem[]>) {
        let max = 0
        days.forEach(day => {
    ...
    getCategoryColor(category: string, allCategories: string[] = []) {
    ...
    // 获取行文本颜色（支持随机颜色模式）
    getRowTextColor(isRandomColor: boolean) {
        return isRandomColor ? new Color(Utils.randomColor16()) : this.widgetColor
    }

    // 渲染模型详情行（抽离公共逻辑）
    renderCategoryDetailRow(w: ListWidget, category: string, spendCents: number, total: number, allCategories: string[], textColor: Color, padding = 50) {
    ...
    // 按类别排序items
    sortItemsByCategory(items: DailySpendItem[], allCategories: string[]) {
    ...
    renderAggregated = async (w: ListWidget) => {
        if (this.aggregatedData && this.aggregatedData.aggregations && this.aggregatedData.aggregations.length > 0) {
            ...
            const validItems = this.aggregatedData.aggregations
                .filter(item => {
                    const hasTokens = parseTokenCount(item.inputTokens) > 0 ||
                ...
                .sort((a, b) => {
                    ...
                    // 如果花费相同，按总 token 数降序
                    const tokensA = (parseTokenCount(a.inputTokens) + parseTokenCount(a.outputTokens) +
                                   parseTokenCount(a.cacheWriteTokens) + parseTokenCount(a.cacheReadTokens))
                    const tokensB = (parseTokenCount(b.inputTokens) + parseTokenCount(b.outputTokens) +
                                   parseTokenCount(b.cacheWriteTokens) + parseTokenCount(b.cacheReadTokens))
            ...
            displayItems.forEach(item => {
                ...
                // Token 数量
                const totalTokens = (parseTokenCount(item.inputTokens) +
                                    parseTokenCount(item.outputTokens) +
                                    parseTokenCount(item.cacheWriteTokens) +
                                    parseTokenCount(item.cacheReadTokens)).toString()
            ...
            const totalTokens = (parseTokenCount(this.aggregatedData.totalInputTokens) +
                                parseTokenCount(this.aggregatedData.totalOutputTokens) +
                                parseTokenCount(this.aggregatedData.totalCacheWriteTokens) +
                                parseTokenCount(this.aggregatedData.totalCacheReadTokens)).toString()
